import { AgeRange } from "@/_lib/constants";
  ...
  _updateFilteredAnimals: () => {
    ...
    const filtered = animals.filter((animal) => {
      ...
      if (filters.ageRanges.length > 0) {
        const isInSelectedRange = filters.ageRanges.some(range =>
          animalAge >= range.min && animalAge < range.max
        );
      ...
      if (filters.incompatibilities.length > 0) {
        ...
        const hasFilteredIncompatibility = filters.incompatibilities.some(id =>
          animalIncompatibilityIds.includes(id)
        );
